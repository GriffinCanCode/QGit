...
def create_safespace(commit_hash: str) -> str:
    ...
    try:
            ...
            if file_path.strip():
                try:
                    ...
                    logger.log(
                        level="warning",
                        command="last",
                        message=f"Failed to copy tracked file {file_path}: {e!s}",
                    )
            ...
            if line.startswith("??"):
                ...
                try:
                    ...
                    logger.log(
                        level="warning",
                        command="last",
                        message=f"Failed to copy {src_file}: {e!s}",
                    )
        ...
        raise GitStateError(f"Failed to create safespace: {e!s}")
...
def checkout_commit(commit_hash: str, safespace_dir: Optional[str] = None) -> None:
    ...
    try:
        ...
        if safespace_dir:
            ...
            if status.strip():
                stash_msg = (
                    f"Temporary stash for qgit last ({datetime.now().isoformat()})"
                )
        ...
        backup_branch = (
            f"backup_{current_branch}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        raise GitStateError(f"Failed to checkout commit: {e!s}")
...
def complete_last(safespace_dir: str) -> None:
    ...
    try:
        ...
        if not (safespace_path / "metadata.json").exists():
            raise GitStateError(
                f"Invalid safespace directory (no metadata): {safespace_dir}"
            )
        ...
        raise GitStateError(f"Failed to clean up safespace: {e!s}")
