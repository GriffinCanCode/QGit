...
def verify_branch() -> bool:
    ...
    try:
        ...
        raise GitStateError(f"Branch verification failed: {e!s}")
...
def check_security() -> Tuple[bool, Dict[str, List[Dict[str, Any]]]]:
    ...
    try:
        ...
        raise GitStateError(f"Security check failed: {e!s}")
...
def handle_risky_files(scan_results: Dict[str, List[Dict[str, Any]]]) -> bool:
    ...
    print("\nWould you like to:")
    print(
        f"{COLORS['CYAN']}1. Update .gitignore and untrack risky files{COLORS['ENDC']}"
    )
    ...
    try:
        ...
        raise GitStateError(f"Failed to handle risky files: {e!s}")
...
def push_to_main() -> bool:
    ...
    try:
        ...
        logger.log(
            ...
            metadata={"timestamp": datetime.now().isoformat(), "branch": "main"},
        )

        return True

    except GitCommandError as e:
        raise GitStateError(f"Push failed: {e!s}")
