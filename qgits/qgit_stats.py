...
def get_team_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        for line in output.split("\n"):
            ...
            if (
                not author_stats["first_commit"]
                or commit_date < author_stats["first_commit"]
            ):
                author_stats["first_commit"] = commit_date
            if (
                not author_stats["last_commit"]
                or commit_date > author_stats["last_commit"]
            ):
        ...
        return {"total_authors": len(authors), "authors": dict(authors)}
    ...
    try:
        ...
        for line in output.split("\n"):
            ...
            if additions.isdigit() and deletions.isdigit():
                ...
                author_stats[current_author]["files_changed"][filename][
                    "additions"
                ] += int(additions)
                author_stats[current_author]["files_changed"][filename][
                    "deletions"
        ...
        sorted_authors = sorted(
            author_stats.items(), key=lambda x: x[1]["total_changes"], reverse=True
        )
