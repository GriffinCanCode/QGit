...
def install_dotenv() -> bool:
    ...
    try:
        ...
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "--user", "python-dotenv==1.0.1"]
        )
        print("✓ python-dotenv installed successfully")
        return True
    except subprocess.CalledProcessError as e:
        print(f"⚠️ Failed to install python-dotenv: {e!s}")
        return False
...
def verify_dotenv_installation() -> bool:
    ...
    try:
        ...
        if not hasattr(dotenv, "load_dotenv"):
            ...
            return False
        print(
            f"✓ python-dotenv version {dotenv.__version__} is installed and functional"
        )
        return True
    except ImportError as e:
        print(f"⚠️ Error importing python-dotenv: {e!s}")
        print(f"Python path: {sys.path}")
        return False
    except Exception as e:
        print(f"⚠️ Error verifying dotenv functionality: {e!s}")
        return False
    ...
    if install_dotenv():
        ...
        if verify_dotenv_installation():
            try:
                ...
                print(f"⚠️ Failed to import python-dotenv after installation: {e!s}")
                DOTENV_AVAILABLE = False
        else:
...
def configure_git_login() -> Tuple[bool, str]:
    ...
    try:
        ...
        return False, f"Error configuring Git login: {e!s}"
...
def configure_sudo_password() -> Tuple[bool, str]:
    ...
    try:
        ...
        sudo_password = getpass.getpass(
            "Enter your sudo password (or press Enter to skip): "
        )
        ...
        return False, f"Error configuring sudo password: {e!s}"
...
def verify_git_setup() -> Tuple[bool, str]:
    ...
    try:
        ...
        if env_path.exists() and DOTENV_AVAILABLE:
            ...
            print(
                "⚠️ python-dotenv not installed. Environment variables will not be loaded."
            )
        ...
        if not user_name or not user_email:
            return (
                False,
                "Git user configuration is missing. Please set up your Git account with:\n"
        ...
        try:
            ...
            if not commit_hash:
                return (
                    False,
                    "Git repository not initialized. Please run 'git init' in your project directory.",
                )
        ...
        return (
            True,
            f"Git setup verified successfully for user: {user_name} <{user_email}>",
        )

    except subprocess.CalledProcessError as e:
        return False, f"Error verifying Git setup: {e!s}"
    except Exception as e:
        return False, f"Unexpected error during Git setup verification: {e!s}"
